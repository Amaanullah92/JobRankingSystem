...
namespace JobRankingSystem.Services
{
    public class AVLTreeService
    {
        ...
        public AlgorithmTrace InsertAndGetTrace(List<Candidate> candidates)
        {
            ...
            {
                _trace.AddStep(new AlgorithmStep
                {
                    StepId = _stepId++,
        ...
        private AVLNode Insert(AVLNode node, Candidate key)
        {
            ...
            {
                _trace.AddStep(new AlgorithmStep { StepId = _stepId++, Description = "Right Rotation (LL Case)" });
            ...
            {
                _trace.AddStep(new AlgorithmStep { StepId = _stepId++, Description = "Left Rotation (RR Case)" });
            ...
            if (balance > 1 && key.Id > node.Left.Data.Id)
            {
                _trace.AddStep(new AlgorithmStep { StepId = _stepId++, Description = "Left-Right Rotation (LR Case)" });
            ...
            if (balance < -1 && key.Id < node.Right.Data.Id)
            {
                _trace.AddStep(new AlgorithmStep { StepId = _stepId++, Description = "Right-Left Rotation (RL Case)" });
